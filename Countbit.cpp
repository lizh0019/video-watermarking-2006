...
void CountBitsMB(int Mode, int COD, int CBP, int CBPB, Pict *pic, Bits *bits) 
{
  ...
  if (pic->picture_coding_type == PCT_INTER) { //输出编码的宏块指示
    putbits(1,COD);
    bits->COD++;
  }
...
int  CountBitsCoeff(int *qcoeff, int Mode, int CBP, Bits *bits, int ncoeffs,int ifout) //输出系数
{
  
  int i,bitnum=0;

  if (Mode == MODE_INTRA || Mode == MODE_INTRA_Q) { //内部块
    for (i = 0; i < 4; i++) {
	  if(ifout)	
      bits->Y += CodeCoeff(Mode, qcoeff,i,ncoeffs,ifout);//输出量化后的dct系数
	  else
...
int FindCBP(int *qcoeff, int Mode, int ncoeffs)
{
  ...
  for (i = 0; i < 6; i++) { //1个亮度块+2个色度块=6*8*8
...
/************************亮度块*****************************/
  for (k=0;k<16;k+=8) { // 此循环完成8×8分块
   ...
   mse=VectorDistance(64,fblock,*(codeword+index[c]));//计算实际失真
...
/************************Cb块*****************************/
  ...
  mse=VectorDistance(64,fblock,*(codeword+index[c]));//计算失真
  mse2=VectorDistance(64,fblock,zero);
  if(mse2<=mse)
	  indicator[c]=0;
  else{
             ...
             int *pmv0, int *pmv1, int block, int newgob, int half_pel)
{
  ...
  l8 = o8 = or8 = 0;
  if (MV[0][y][x-1]->Mode == MODE_INTER4V) //同行左邻
    l8 = 1;
  if (MV[0][y-1][x]->Mode == MODE_INTER4V) // 同列上邻
    o8 = 1;
  if (MV[0][y-1][x+1]->Mode == MODE_INTER4V)// 右上角
    or8 = 1;
  ...
  *pmv0 = p1+p2+p3 - mmax(p1,mmax(p2,p3)) - mmin(p1,mmin(p2,p3));// 水平分量预测值
  ...
  *pmv1 = p1+p2+p3 - mmax(p1,mmax(p2,p3)) - mmin(p1,mmin(p2,p3)); //垂直分量预测值
              ...
              int x, int y, int Mode, int newgob, Pict *pic,int ifout )
{
  ...
  for (block = start; block <= stop;  block++) {
    ...
    x_vec = (2*MV[block][y][x]->x + MV[block][y][x]->x_half) - pmv0;//插值后的运动矢量减去预测值
