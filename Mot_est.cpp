...

{
  ...
  search_area = LoadArea(prev, ilow, jlow, h_length, v_length, lx);  //装入搜索范围块：初始ilow,jlow;大小h_length, v_length
  ...
  if (x_curr-ilow         < 0        || y_curr-jlow         < 0        ||
    ...
    *SAD_0 = SAD_Macroblock(ii, act_block, h_length, Min_FRAME[0]) - //计算搜索区域中当前块位置的宏块与当前块之间的差值
       PREF_NULL_VEC;
  }
      ...
      for (k = 0; k < 8*l; k++) {
        if (i>=adv_ilow && i<=adv_ihigh && j>=adv_jlow && j<=adv_jhigh) {
          ...
          for (block = 0; block < 4; block++) {
            ...
            aa = act_block + ((block&1)<<3) + ((block&2)<<2)*16;
            sad = SAD_Block(ii, aa, adv_h_length, Min_FRAME[block+1]);//失真函数
				 ...
				 PictImage *prev_recon,int *SAD_0,int newgob)
{
 ...
 /* 螺旋搜索 */
  for (l = 1; l <= sxy; l++) {
    ...
    for (k = 0; k < 8*l; k++) {
      if (i>=ilow && i<=ihigh && j>=jlow && j<=jhigh) {
		 ...
		 //*********************************R-D optimization*********************************************
          Bits  *tempbit;
          //mvbit=CountBitsVectors(MV, tempbit, x_curr/MB_SIZE, y_curr/MB_SIZE, Mode, newgob, pic,0);//计算矢量数据比特数
          dctbit=(MV[0][y_curr/MB_SIZE+1][x_curr/MB_SIZE+1]->min_error)/QP-80;// 预测DCT的系数*/
  ...
  {
   inform.Format("最优运动矢量x=%d,y=%d,\n最优失真=%d,最优失真比特数=%d,最优失真MV比特数=%d,\n最小失真=%d,最小失真比特数=%d,最小失真MV比特数=%d",MV_FRAME[0].x,MV_FRAME[0].y,Min_FRAME[0],m5,m,m3,temptest2,temptest);
