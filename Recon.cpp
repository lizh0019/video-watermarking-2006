...
void CRecon::reconstruct(int bx, int by, int P, int bdx, int bdy)// bx,by块的位置，bdx，y运动矢量
{
  ...
  else 
  {
    ...
    if (adv_pred_mode) 
	{
      if (mode == MODE_INTER4V) 
	  {
	     ...
	     for (comp = 0; comp < 4; comp++) 
		 {
	        xvec += dx = (trb)*MV[0][comp+1][y][x]/trd + bdx; //trb时间参考
...
void CRecon::recon_comp(unsigned char *src, unsigned char *dst, 
    int lx, int lx2, int w, int h, int x, int y, int dx, int dy, int flag)// dx,dy运动矢量
{
 int xint, xh, yint, yh;
  unsigned char *s, *d;
  ...
  if (!xh && !yh)
    if (w!=8)
      rec(s,d,lx,lx2,h);// 不插值
    else
      recc(s,d,lx,lx2,h);// 色度
  else if (!xh && yh)
    if (w!=8)
      recv(s,d,lx,lx2,h);// 垂直插值
    else 
      recvc(s,d,lx,lx2,h);
  else if (xh && !yh)
    if (w!=8)
      rech(s,d,lx,lx2,h);// 水平插值
    else
      rechc(s,d,lx,lx2,h);
  else /* if (xh && yh) */
    if (w!=8)
      rec4(s,d,lx,lx2,h);// 双向插值
    else
      rec4c(s,d,lx,lx2,h);
