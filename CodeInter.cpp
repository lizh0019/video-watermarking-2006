				  ...
				  int *embedtimes, int *P_frg_times,bool mvmark)
{
	...
	/* 整数和半象素运动估值 */
	if((mvmark)&&(*embedtimes<4096))// 运动矢量嵌水印
	...
	for ( j = 0; j < lines/MB_SIZE; j++) {
	  ...
	  for ( i = 0; i < pels/MB_SIZE; i++)
	  {
		...
		CBP = FindCBP(qcoeff_P, Mode, 64); //计算6个块中有几个块需要传送
           ...
           MotionVector *MV[6][MBR+1][MBC+2], int gobsync)
{
  ...
  for ( j = 0; j < lines/MB_SIZE; j++) { //行循环
    ...
    for ( i = 0; i < pels/MB_SIZE; i++) { // 列循环
      ...
      FindPMV(MV,i+1,j+1,&pmv0,&pmv1,0,newgob,0); // 预测运动矢量
      ...
      MotionEstimation(curr, prev, i*MB_SIZE, j*MB_SIZE, //宏块运动估计
		  xoff, yoff, seek_dist, MV, &sad0);

      sad16 = f0->min_error;/*16×16块的最小失真*/
      if (advanced)
        sad8 = f1->min_error + f2->min_error + f3->min_error + f4->min_error;/*8×8块的最小失真*/

      f0->Mode = ChooseMode(curr,i*MB_SIZE,j*MB_SIZE, mmin(sad8,sad16)); // 选择帧内，帧间编码
	...
	for ( j = 0; j < lines/MB_SIZE; j++) { //行循环
		...
		for ( i = 0; i < pels/MB_SIZE; i++) { // 列循环
			...
			if (f0->Mode != MODE_INTRA) {
	            ...
	            if((fabs(f0->x)<13)&&(fabs(f0->x)>2)&&(fabs(f0->y)<13)&&(fabs(f0->y)>2)&&(*(embedtimes)*3<=4096)) //嵌水印
				{   
					if((*embedtimes)==116)
                    {
                       CString test;
                    ...
                    if(indicator==0)
				    MV_WatermarkEmbed(f0,secondtribit+(*embedtimes),f0->Mode);//嵌入 
					if(indicator==1)
                    MV_WatermarkEmbed(f0,secondtribit+(*embedtimes),f0->Mode);//嵌入(多功能)
					ifwater=true;
					///////////////// For Test Only //////////////////////
					/*CString test;
